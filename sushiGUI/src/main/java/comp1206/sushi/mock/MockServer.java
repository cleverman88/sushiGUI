...
public class MockServer implements ServerInterface {
	...
	public MockServer() {
		...
		//Choosing a random starting position from the suppliers
		Random randomizer = new Random();
		Postcode random = (getSuppliers().get(randomizer.nextInt(getSuppliers().size()))).getPostcode();
		Postcode random2 = (getSuppliers().get(randomizer.nextInt(getSuppliers().size()))).getPostcode();
		Postcode random3 = (getSuppliers().get(randomizer.nextInt(getSuppliers().size()))).getPostcode();
		//Setting the source and destination for the drones
		Drone drone1 = addDrone(1);
		Drone drone2 = addDrone(2);
		Drone drone3 = addDrone(3);
		drone1.setSource(random);
		drone2.setSource(random2);
		drone3.setSource(random3);
		drone1.setDestination(restaurantPostcode);
		drone2.setDestination(restaurantPostcode);
		drone3.setDestination(restaurantPostcode);
